function Diagnose = missioncompare_version_final(DT_Output, MI_Output)
% missioncompare
...
%     - First prints overall result
%     - Then prints every checked variable with PASS/FAIL and result number
...
if isempty(fieldnames(Diagnose.Failures))
    fprintf('OVERALL RESULT: ALL VARIABLES WITHIN ACCEPTABLE LIMITS\n\n');
else
    fprintf('OVERALL RESULT: FAULT DETECTED\n\n');

    fprintf('FAILED VARIABLES:\n');
...
fprintf('VARIABLE-BY-VARIABLE RESULTS:\n');
fprintf('------------------------------------------------------------\n');
fprintf('%-20s %-10s %-15s\n', 'Variable', 'Result', 'Diff Score');
fprintf('------------------------------------------------------------\n');
...
for i = 1:length(checkedVars)
    ...
    if isfield(Diagnose.Failures, varNameChar)
        resultText = 'FAIL';
    else
        resultText = 'PASS';
    end

    fprintf('%-20s %-10s %-15.2f\n', varNameChar, resultText, diffScore);
end
